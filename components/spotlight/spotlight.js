  ...
  for (let i = 0; i < items.length; i++) {
    ...
    stories.push({
      ...
      seeMore: ({ close }) => (
        <div
          className={
            'bg-gray-50 text-gray-800 max-w-full w-full h-full p-6 absolute top-0 left-0'
          }
        >
          <div
            onClick={close}
            className={
              'absolute top-2 right-2 w-6 h-6 text-burgundy rounded-full cursor-pointer'
            }
          >
            <CloseIcon />
          </div>
          {img.readMore?.json && (
            <RichtextComponent content={img.readMore.json} />
          )}
        </div>
      ),
            ...
            >
              <div className={'absolute top-0 left-0 opacity-0'}>
                <Image
                  ...
                  alt={
                    items[nextImage].image.description || 'No alt text found'
                  }
                />
              </div>
              <Image
              ...
              <div
                className={
                  'opacity-0 hover:opacity-100 absolute top-0 left-0 w-1/2 bg-gradient-to-r from-gray-800 to-transparent h-full'
                }
              />
              <div
                className={
                  'opacity-0 hover:opacity-100 absolute top-0 left-1/2 w-1/2 bg-gradient-to-l from-gray-800 to-transparent h-full'
                }
              />
            </motion.div>
          </WithSeeMore>
